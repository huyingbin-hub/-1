	...
	<view class=" two">
		...
		<view class="content">
			...
			<uni-popup class="model-km" ref="popup" type="top">
				<view class="km-content" style="background-color: #FFFFFF;width: 100%;">
					...
					<view class="km-list">
						<view v-bind:class="{activekm:kmactive==index}" @click="choseKm(index,item)" v-for="(item,index) in kmList" :key='index'
						 class="km-text">{{item.lb}}</view>
					</view>
			...
			<view style="">
				<scroll-view class="scroll-view_H" scroll-x="true" :scroll-with-animation="true" @scroll="scroll" scroll-left="0">
					<view @click="choseNow(index,item)" v-bind:class="{active:isactive==index}" class="scroll-view-item" v-for="(item,index) in kcList"
					 :key="index">
		...
		<view class="no-login" v-if="userKey==''">
			<view>
				...
				<button style="background-color: #FFFFFF;color: #000;" open-type="getPhoneNumber" v-if="phone==''" @getphonenumber="getPhoneNumber"
