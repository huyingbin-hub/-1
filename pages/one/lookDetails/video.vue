<template>
	...
	<view class="look-details">
		...
		<view class="video" style="width: 100%;">
			...
			<!-- <videoPlayer :course_info="detailsKc" :video_info="video_info" :course_auth_status="course_auth_status">
			</videoPlayer> -->
	...
	export default{
		data(){
		  ...
		  onLoad: function (option) { //option为object类型，会序列化上个页面传递的参数
				...
				this.url = `http://localhost:8080/#/?kc_id=${option.id}&user_id=${option.key}`
				//获取到课程id
				this.kcId=option.id;
...
<style lang="scss">
	...
	.look-details{
		...
		.class-name{
			...
			line-height: 20px;
			.class-info{
				text{color: #999999;font-size: 10px;margin-right: 15px;}
			}
		}
		.class-all{
			.mu.active text{color: #FF6900!important}
			.mu.active .line{height: 2px;background-color: #FF6900;width: 10%;text-align: center;margin: 10px 45%;}
			padding: 10px 16px;
			.class-tab{
				display: flex;
				.mu{
		...
		.class-button{
			...
			width: 100%;
			overflow: hidden;
			.zx.active text{color: #FFFFFF!important;}
			.zx{
				width: 40%;
				float: left;
				height: 44px;
				...
				image{width: 16px;height: 16px;margin-top: 5px;}
				text{
					font-size: 12px;
					color: #999999;
					display: block;
			...
			.buy.active text{color: #FFFFFF!important;}
			.buy{
				height: 44px;
				line-height: 44px;
				overflow: hidden;
