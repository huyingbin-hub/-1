<template>
	<view class="content">
		...
		<view class="km-content" style="background-color: #FFFFFF;width: 100%;">
			...
			<view class="km-list">
				<view v-bind:class="{activekm:kmactive==index}" @click="choseKm(index,item)" v-for="(item,index) in kmList" :key='index' class="km-text">{{item.lb}}</view>
			</view>
		...
		<view style="">
		 <scroll-view id="scrollBody"
			 class="scroll-view_H" 
			 scroll-x="true" 
			 :scroll-with-animation="true" 
			 lower-threshold='60'
			 @scroll="scroll" scroll-left="0">
				 <view @click="choseNow(index,item)"  v-bind:class="[{active:isactive==index},{active1:isactive2}]" class="scroll-view-item" v-for="(item,index) in kcList" :key="index">
	...
	export default {
		...
		methods: {
			...
			choseXm(index,data){
				...
				// uni.setStorageSync('kmData',this.kmList[0]);
				/* this.kmChose=this.kmList[0].lb; */
				/* this.kmChose=this.xmList[index].child[0].lb;
				console.log(this.xmList[index].child,'=====') */
				this.isactive=0;
				this.xmIndex=index;
			...
			choseKm(index,data){
				...
				console.log(this.kmChose)
				this.kmactive=index;
				console.log(this.kmactive)
	...
	.content {
		.hd{
		 ...
		 .no-login{
			 ...
			 image{width: 99px;height: 106px;}
			 text{margin-top: 10px;font-size: 12px;color: #999999;}
			 button{width: 115px;
					height: 30px;
					line-height: 30px;
					font-size: 14px;
		...
		.model-km{
			.km-content{
				padding-bottom: 10px;
				.title{font-size: 14px;margin-left: 16px;padding-top: 10px;}
				.km-list{
					display: flex;
					width: 100%;
					flex-wrap: wrap;
					padding: 10px 18px;
					margin: 0 -10px;
					.km-text{font-size: 12px;width: 20%;line-height: 26px;
							margin:5px 6px;
		...
		.xm-chose{
			.all{
				...
				height: 100%;
				font-size: 14px;
				.left{float: left;width: 120px;padding:5px 20px 5px 0;margin-left: 5px;
				height: 100%;
				position: relative;
				...
				view{
					...
					margin-bottom: 29px!important;
					  ...
					  font-weight: bold;
					margin: 10px 0;
					&.active{color: #FF6900;border-left: 2px solid #FF6900;}
		...
		.chose-content{
			...
			.chose{
				...
				image{width: 12px;height: 12px;vertical-align: middle;
				display: inline-block;}
			}
		}
		.box{display: flex;flex-wrap: wrap;margin-bottom: 20px;
		&.active{
			opacity: 0;
			}
		}
		.class-details{
			...
			.class-img{
				...
				image{
				width: 100%;border-radius: 5px;height: 95px;
			}}
			.class-info{
				padding: 10px;
				.title{font-size: 12px;width: 100%;white-space: nowrap;
						overflow: hidden;
						text-overflow: ellipsis;}
				.info{
					display: flex;
					justify-content: space-between;
					text{font-size: 8px;color: #999999;line-height: 25px;}
				}
				.number{
					display: flex;
					justify-content: space-between;
					.people-num{
						image{width: 14px;height: 14px;margin-right: 5px;vertical-align: middle;}
						text{font-size: 8px;color: #999999;}
					}
					.state{color: #F54130;font-size: 10px;line-height: 25px;}
				}
			}
		}
	}
