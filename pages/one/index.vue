<template>
	<view class="content">
		...
		<view class="xm-chose">
			<drawer :show="drawerShow" direction="right">
				<view class="all" v-if="drawerShow">
					...
					<view class="right">
						<view @click="choseXmchild(index, i)" :class="{ active: xmChildactive == index }" v-for="(i, index) in xmList[xmIndex].child" :key="index">{{ i.lb }}</view>
					</view>
				</view>
			</drawer>
		</view>
		<!--返回顶部-->
		<view class="wrap"><u-back-top :scroll-top="scrollTop"></u-back-top>
		...
		<view style="">
			<scroll-view id="scrollBody" class="scroll-view_H" scroll-x="true" :scroll-with-animation="true" lower-threshold="60" @scroll="scroll" scroll-left="0">
				<view
					@click="choseNow(index, item)"
					v-bind:class="[{ active: isactive == index }, { active1: isactive2 }]"
		...
		<view class="box" v-bind:class="{ active: isactive1 }">
			<view @click="godetails(i)" class="class-details" v-for="(i, index) in classList" :key="index">
				<view class="class-img"><image :src="i.pic"></image></view>
			...
			<uni-popup class="model-km" ref="popupBuy" type="bottom">
				<view class="km-content" style="background-color: #FFFFFF;width: 100%;">
					<view class="buy-content">
						...
						<view
							style="width: 100%;text-align: center;font-size: 14px;margin: 10px 0;white-space: nowrap;
						overflow: hidden;
		...
		<view v-if="showLoad" style="width: 100%;text-align: center;"><image style="width: 40px;height: 40px;margin: 10px 0;" src="../../static/loading.gif"></image></view>
	</view>
</template>
